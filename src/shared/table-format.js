...
(function(root, factory) {
    ...
    function format(rows, alignments = [], style = 'aligned', explicitLeft = []) {
        if (!rows.length) return '';
        const compact = normalize(style) === 'compact';
        const columnCount = rows[0].length;
        const markers = Array.from({ length: columnCount }, (_, i) =>
            alignments[i] === 'center' ? ':---:' : alignments[i] === 'right' ? '---:' : explicitLeft[i] ? ':---' : '---');
        ...
        const separators = compact ? markers : markers.map((marker, i) => {
            if (marker.startsWith(':') && marker.endsWith(':')) return ':' + '-'.repeat(widths[i] - 2) + ':';
            if (marker.startsWith(':')) return ':' + '-'.repeat(widths[i] - 1);
