...
const en = {
    ...
    unsupportedHost: 'Experimental export requires local desktop VS Code on macOS, Linux or Windows.',
    unsupportedRemote: 'Export is not yet supported in remote windows, including Remote-SSH. Open a local copy of the Markdown file and its referenced assets in desktop VS Code on macOS, Linux or Windows to export.',
...
const translations: Record<string, ExportMessages> = {
    ...
    ja: {
        ...
        unsupportedHost: '試験提供のエクスポートには、macOS、Linux、Windows上のローカルデスクトップ版VS Codeが必要です。', trustRequired: 'エクスポートするには、このワークスペースを信頼してください。',
