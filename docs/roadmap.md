...
| Release automation | One identified VSIX, Ubuntu/macOS/Windows and minimum-VS-Code checks, checksums, validation evidence and a manually published release draft | Marketplace/Open VSX distribution and further validated environments |
...
[PR #7](https://github.com/BinaryOutlook/binary-markdown-fork/pull/7) implements the white-PDF default and full-page theme background correction tracked in [issue #6](https://github.com/BinaryOutlook/binary-markdown-fork/issues/6). The [bug report and validation receipt](../reports/investigations/2026-09-12-export-page-background.md) retain the original investigation and post-fix evidence. DOCX checks did not reproduce the page-background defect, so its page styling is unchanged. This dated report does not establish the current publication status; see [GitHub Releases](https://github.com/BinaryOutlook/binary-markdown/releases) for released source.
...
Windows x86-64 export and its required CI lane are implemented in the integration workstream; actual Windows execution is pending in the [initial preparation record](../reports/validation/2026-09-14-windows-preparation.md). VS Code Remote/SSH, browser-hosted VS Code and Electron export remain outside the supported local export scope. Running local desktop VS Code under Xvfb on an Ubuntu machine accessed by SSH tests the Ubuntu local extension host; it does not establish Remote-SSH export support.
