...
The next PDF appearance work is recorded in the [page-background bug and feasibility assessment](export-page-background-2026-09-12.md): add a white-output preference and fill the full page when retaining a non-white theme. The PDF correction and white-default setting are implemented on the [issue #6](https://github.com/BinaryOutlook/binary-markdown/issues/6) branch, with final validation in progress before review. The corresponding DOCX checks did not reproduce the page-background defect.
