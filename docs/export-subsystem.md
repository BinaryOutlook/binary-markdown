...
| --- | --- |
| Required host | Local desktop VS Code on macOS first. Keep shared editor/Electron builds compatible; other environments require later acceptance. |
...
| NFR-EXP-008 | **Host compatibility:** The packaged subsystem shall operate in the declared local macOS VS Code environment using only its shipped assets and explicitly installed dependencies. | User | AC-14 |
...
| AC-14 | Install the actual VSIX in an isolated local macOS VS Code profile, stop the development server, record resolved external tools, and reproduce the native entry/save/export flows. Missing-dependency cases remain part of this check. | D2 early smoke; D5 final |
