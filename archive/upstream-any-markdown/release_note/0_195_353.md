...
b は親リストの **最初の項目**なので、b の子 (c) は d より **前**に来なければならない。 `appendChild` は末尾に追加するため c が d の後ろに移動してしまっていた。

#### 2. テキストノード非正規化

b のテキスト "b" を parentLi に移動した後、`Node.normalize()` を呼ばなかったため、 "a" (元の textNode) と "b" (移動した textNode) が別ノードとして残っていた。

#### 3. タスクリスト項目の先頭スペース

タスクリスト項目の HTML は `<li><input type="checkbox"> b<ul>...</ul></li>` となっており、 checkbox の直後にスペースを含む textNode `" b"` がある。 マージ後に `normalize()` しても "a b"（スペースあり）になってしまっていた。
