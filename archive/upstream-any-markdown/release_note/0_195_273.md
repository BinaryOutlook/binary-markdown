...
コードブロックの言語に `math` を指定すると、KaTeX を使った数式レンダリングが行われるようになった。Mermaidブロックと同様の描画モード/編集モード切り替えUIを持つ。 また、矢印キーによる要素間ナビゲーションで、空の段落（`<p><br></p>`）が特殊ブロック（Mermaid/Math/コードブロック）の直前にある場合にカーソルが正しく移動しない問題を修正した。
