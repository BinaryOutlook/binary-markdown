...
- Use **Binary Markdown: Copy Build Information** to report the exact source revision, local changes and host environment. Reports from older development commits are welcome. See [releases and support](../docs/releases-and-support.md).
- Adopt the supplied 01 / MD artwork and **AGPL-3.0-or-later** for current development. Earlier MIT versions, inherited notices and third-party licenses are preserved; see [licensing](../docs/licensing.md) and [branding](../docs/branding.md).
- Keep `BinaryOutlook.binary-markdown` and its settings/storage identifiers when updating from 0.1.0. Reopen editor tabs or reload VS Code after installation. [Older ID migration](../docs/migration.md) is separate.

The release scope is local desktop VS Code on macOS and Ubuntu. Required automated validation checks the same VSIX on Ubuntu/macOS and in the declared minimum VS Code 1.85.0; evidence and any remaining manual-reader limitations are linked in [validation](../reports/validation/0.2.0.md). Windows export, Remote/web hosts and standalone installers remain future work.
