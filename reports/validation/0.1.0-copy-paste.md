...
Historical record; the original recording date was not specified. The baseline was upstream `main` at `8248742`, with [code-copy PR #8](https://github.com/raggbal/any-markdown/pull/8) and [outline-state PR #9](https://github.com/raggbal/any-markdown/pull/9) combined. The observations below predate current integration work and do not certify a later package. Use the [manual fixture](../../test/fixtures/manual/copy-paste.md) for a new check. The fixture now uses synthetic output instead of commands that print a username, directory or file listing.
