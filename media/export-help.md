...
- `binary-markdown.export.showCodeLanguage` controls language badges in DOCX and direct PDF exports and is enabled by default. Turn it off in User or Workspace Settings to hide badges while keeping code block formatting. Changes apply to the next export.
- The gear beside the image-directory control opens Binary Markdown's settings in VS Code. The image icon opens image-directory settings.
