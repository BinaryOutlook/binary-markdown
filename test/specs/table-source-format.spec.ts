...
for (const mode of ['aligned', 'compact']) {
    test(`${mode}: pipes in code spans and link destinations survive a neighboring cell edit`, async ({ page }) => {
        const source = '| Code | Nested | Link | Value |\n| --- | --- | --- | --- |\n' +
            '| `a\\|b` | **`c\\|d`** | [link](https://example.org/a\\|b) | 1 |\n';
        await message(page, { type: 'tableSourceFormat', value: mode });
        await message(page, { type: 'update', content: source });
        await expect(page.locator('#editor td code')).toHaveText(['a|b', 'c|d']);
        await editCell(page, '2', 0, 3);
        const saved = await save(page);
        expect(saved).toContain('`a\\|b`');
        expect(saved).toContain('**`c\\|d`**');
        expect(saved).toContain('[link](https://example.org/a\\|b)');
        await page.reload();
        await page.waitForFunction(() => (window as any).__testApi?.ready);
        await message(page, { type: 'update', content: saved });
        await expect(page.locator('#editor td')).toHaveCount(4);
        await expect(page.locator('#editor td code')).toHaveText(['a|b', 'c|d']);
        await expect(page.locator('#editor td a')).toHaveAttribute('href', 'https://example.org/a|b');
        expect(await save(page)).toBe(saved);
    });

    test(`${mode}: table edits retain block spacing, prose wraps, hard breaks, and neighboring source`, async ({ page }) => {
        const prefix = '\n## Probe\n\n\nSoft\nwrap\n\nHard  \nbreak\n\nBackslash\\\nbreak\n\n\n';
        const suffix = '\n\n3. First\n\n   Continued\n\n4. Second\n\n' + otherTable + '\nAfter\n\n';
        const original = prefix + table + suffix;
        const formatted = (mode === 'aligned' ? aligned.replace('3     ', '30    ') : compact.replace('3 |', '30 |'));
        const expected = prefix + formatted + suffix;
        await message(page, { type: 'tableSourceFormat', value: mode });
        await message(page, { type: 'update', content: original });
        await expect(page.locator('#editor > *')).toHaveCount(8);
        await editCell(page, '30');
        expect(await save(page)).toBe(expected);
        await message(page, { type: 'toggleSourceMode' });
        await expect(page.locator('#sourceEditor')).toHaveValue(expected);
        await message(page, { type: 'toggleSourceMode' });
        await expect(page.locator('#editor > *')).toHaveCount(8);
        await expect(page.locator('#editor > p br')).toHaveCount(2);
        await message(page, { type: 'tableSourceFormat', value: mode === 'aligned' ? 'compact' : 'aligned' });
        await editProse(page);
        expect(await save(page)).toBe(expected.replace('After', 'After!'));
    });
}

test('partial copying across a table excludes unselected source from both clipboard formats', async ({ page }) => {
    const source = 'Before\n\n| Name | Hidden heading |\n|---|---|\n| Select | Hidden value |\n\nAfter\n';
    await message(page, { type: 'update', content: source });
    const copied = await page.evaluate(() => {
        ...
        document.getElementById('editor')!.dispatchEvent(new ClipboardEvent('copy', {
            clipboardData: data, bubbles: true, cancelable: true
        }));
        return { text: data.getData('text/plain'), html: data.getData('text/html') };
    });
    expect(copied.text).toContain('Select');
    expect(copied.html).toContain('Select');
    expect(copied.text).not.toContain('Hidden value');
    expect(copied.html).not.toMatch(/Hidden value|data-md-source|data-table-(source|state)/);
});
