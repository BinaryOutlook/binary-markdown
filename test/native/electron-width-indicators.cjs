...
async function main() {
    ...
    const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'bm-electron-indicators-'));
    ...
    const fixture = path.join(directory, 'indicators.md'); fs.writeFileSync(fixture, source);
    ...
    const evidence = path.join(root, '.vscode-test', 'electron-indicators-' + Date.now()); fs.mkdirSync(evidence, { recursive: true });
    const receipts = [];
    ...
    try {
        ...
        const set = async (key, value) => {
            ...
            if (key === 'editorWidthIndicators') { if (await control.isChecked() !== value) await control.click(); }
            else if (key !== 'editorMaxWidth') await control.selectOption(value);
            else { await control.fill(String(value)); await control.press('Tab'); }
        };
        const before = await indicatorChecks({ editor: page, set, until, record: (name, details) => receipts.push({ name, ...details }) });
        ...
        await application.evaluate(() => {
            ...
            global.__widthStore.prototype.set = function(key, ...args) {
                if (key === 'editorWidthIndicators') { global.__widthStore.prototype.set = original; throw new Error('Synthetic preference failure'); }
                return original.call(this, key, ...args);
            };
        });
        await set('editorWidthIndicators', false);
        await preferences.locator('#settingsError').waitFor({ state: 'visible' });
        assert.equal(await preferences.locator('#editorWidthIndicators').isChecked(), true);
        assert.equal(JSON.parse(fs.readFileSync(path.join(profile, 'config.json'))).editorWidthIndicators, true);
        await set('editorWidthIndicators', false);
        await preferences.locator('#settingsError').waitFor({ state: 'hidden' });
        await page.waitForFunction(() => document.getElementById('editorWidthGuide').hidden);
        await page.evaluate(() => {
        ...
        await set('editorWidthIndicators', true);
        await page.waitForFunction(() => !document.getElementById('editorWidthGuide').hidden);
        ...
        for (const zoom of [1, 2]) for (const width of [500, 1400]) {
            ...
            await until(async () => page.evaluate(() => { const pane = document.getElementById('editorWrapper'); const column = document.getElementById('editor').getBoundingClientRect(); return (document.getElementById('editorWidthGuide').dataset.capped === 'true') === (pane.clientWidth - column.width > 0.5); }), 'resized width marks');
        ...
        await reopened.waitForFunction(() => document.getElementById('editorWidthGuide').hidden);
        ...
        console.log('Electron width indicator checks passed. Evidence:', evidence);
    } finally {
