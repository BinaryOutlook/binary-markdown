...
Use `--suite table-placement` for all eight placements selected once through the installed picker at user and workspace scope, rapid choices, retained selection, reopening, and table action/undo checks. The guarded driver permits workspace writes only for this setting within its owned workspace and removes that override afterward.

For Electron table checks, build the root and Electron as above, then run `node test/native/electron-table-placement.cjs`. This checks the real Preferences window and editor picker, settings persistence, an injected write failure and retry, and retained selection/undo. It requires an unlocked desktop for native menu focus. `--picker-only` exercises the editor picker and its real IPC/storage path without the Preferences window; its receipt explicitly excludes Preferences coverage. Both modes isolate settings, retain a receipt under `.vscode-test/`, and close their own app.
