...
Use `--suite table-placement` for all eight placements selected once through the installed picker at user and workspace scope, rapid choices, retained selection, reopening, and table action/undo checks. It also constrains the installed editor frame to exercise horizontal and vertical overflow, complete visible buttons, all actions remaining reachable, open-menu resizing, Escape, and one-step undo. This is a frame-resize probe; actual OS-window resizing remains a manual check because VS Code's debugging interface does not expose that command. Receipts record the observed webview dimensions. The guarded driver permits workspace writes only for this setting within its owned workspace and removes that override afterward.
