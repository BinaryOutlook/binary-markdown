...
- **UI:** three full/Chinese → simple/English configuration cycles with toolbar placement and keyboard navigation, runtime labels independent of native English VS Code, and GitHub/night HTML exports. The harness reconnects to the visible editor only after `data-export-ready="true"`, then checks the requested toolbar mode and label; static English markup alone is not readiness. Keydown/keyup pairs and observable focus/menu state replace immediate focus assertions. Current full runs produce 43 scenario receipts plus the frozen-input check; the 39-scenario receipt above predates these repeated cycles.
