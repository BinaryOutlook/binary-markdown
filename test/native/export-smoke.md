...
Use `--suite table-placement` for all eight placements selected once through the installed picker at user and workspace scope, rapid choices, retained selection, reopening, and table action/undo checks. It also resizes the owned VS Code window to exercise horizontal and vertical overflow, complete visible buttons, all actions remaining reachable, open-menu resizing, Escape, and one-step undo. Receipts record the observed webview dimensions because host chrome and window minimums affect the available editor size. The guarded driver permits workspace writes only for this setting within its owned workspace and removes that override afterward.
