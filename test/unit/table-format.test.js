...
test('both formats distinguish explicit left alignment from the default header alignment', () => {
    const rows = [['Explicit', 'Default'], ['one', 'two']];
    for (const style of ['aligned', 'compact']) {
        const output = format(rows, ['left', 'left'], style, [true, false]);
